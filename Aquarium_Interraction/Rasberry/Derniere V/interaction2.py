...
# Seuil de mouvement en x et en z
threshold_x = 0.07
threshold_z = 0.07
...
def send_motor_command(dx, dz, dt):
    ...
    if motor_steps_x != 0 :
        command_x = 'DROITE' if motor_steps_x < 0 else 'GAUCHE'
        direction_x = '+' if motor_steps_x < 0 else '-'
    ...
    if motor_steps_z != 0:
        command_z = 'DEVANT' if motor_steps_z < 0 else 'DERRIERE'
        direction_z = '+' if motor_steps_z < 0 else '-'
    ...
    #ser.write(f'I,{motor_steps_x},{motor_steps_z},{dt}\n'.encode())
    print(f'I,{motor_steps_x},{motor_steps_z},{dt}\n')
...
while cap.isOpened():
    ...
    if results.pose_landmarks:
        ...
        #cv2.putText(frame, f'Epaule gauche: x={x:.2f}, z={z:.2f}',(10,30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,0),2)
