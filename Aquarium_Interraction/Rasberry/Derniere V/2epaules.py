...
while cap.isOpened():
    ...
    if results.pose_landmarks:
        ...
        x_sh_l = shoulder_left.x
        z_sh_l = shoulder_left.z
        
        shoulder_right = results.pose_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_SHOULDER]
        x_sh_r = shoulder_right.x
        z_sh_r = shoulder_right.z
        
        x_sh = (x_sh_l + x_sh_r)/2
        z_sh = (z_sh_l + z_sh_r)/2
        
        # Temps
        current_time = time.time()
        
        # Afficher les coordonnées de l'epaule gauche sur l'image 
        cv2.putText(frame, f'Epaules moyenne: x={x_sh:.2f}, z={z_sh:.2f}',(10,30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,0),2)
        ...
        if previous_x is not None and previous_z is not None :
            ...
            previous_x, previous_z = x_sh, z_sh
